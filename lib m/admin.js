...
function addNotification(message, type = "info") {
  ...
  list.unshift({ message, type, time: formatNowTime() });
...
function renderActivityTimeline(issues = []) {
  ...
  const issueEvents = normalizedIssues.flatMap(issue => {
    ...
    if (issue.issueDate) {
      base.push({
        type: "issued",
        ts: Date.parse(String(issue.issueDate)),
        label: `Issued book ${bookId} to ${rollNo}`,
        time: String(issue.issueDate)
      });
    }

    if (isIssueReturned(issue) && issue.returnDate) {
...
async function addStudent(){
  ...
  if (trimmedPhone && !/^\\d{9}$/.test(trimmedPhone)) {
    showErrorPopup("Phone number must be exactly 9 digits.");
    return;
  }
  ...
  const student = {
    ...
    password,
    phone: trimmedPhone,
    email: String(document.getElementById("sEmail")?.value || "").trim()
  };
...
function isIssueReturned(issue) {
  ...
  const hasReturnDate = issue?.returnDate != null && String(issue.returnDate).trim() !== "";
  return status === "RETURNED" || hasReturnDate;
}
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (res.ok) {
        ...
        lastError = "Issues API response is not an array.";
        continue;
      }
      lastError = await res.text();
    } catch (err) {
...
async function loadDashboardOverview() {
  ...
  const [books, students, staffs, issues] = await Promise.all([
...
async function loadIssuedBooks() {
  ...
  try {
    ...
    console.error(err);
    tbody.innerHTML = makeEmptyStateRow(10, "Failed to load issued books", "Check backend issue API and try again.", "Alert");
  }
}
...
async function loadReturnedBooks() {
  ...
  try {
    ...
    console.error(err);
    tbody.innerHTML = makeEmptyStateRow(12, "Failed to load returned books", "Please retry after checking server status.", "Alert");
  }
}
...
async function loadPendingFines() {
  ...
  try {
    ...
    console.error(err);
    tbody.innerHTML = makeEmptyStateRow(7, "Failed to load pending fines", "Verify backend issue/fine API.", "⚠️");
  }
}
